...
export const useScheduleEditRow = (schedule: ScheduleDTO) => {
  ...
  const deletingIds = useAtomValue(deletingScheduleIdsAtom);

  // 同時削除でも対象行だけ loading にする（pending な全削除の id を見る）
  const removing = deletingIds.includes(schedule.id);
